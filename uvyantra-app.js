// ================================================================
// КОНФИГУРАЦИЯ — вставьте URL вашего Apps Script после деплоя
// ================================================================
// ================================================================
// КОНФИГУРАЦИЯ
// После деплоя замените значения на реальные
// ================================================================
const BACKEND_URL = 'http://localhost:3000'; // ← заменить на https://api.uvyantra.ru
const DEMO_MODE   = BACKEND_URL.includes('localhost');
const API_URL     = BACKEND_URL;

// Firebase конфигурация — вставьте из Firebase Console
...
const FIREBASE_CONFIG = {
  ...
  vapidKey:          'ВСТАВЬТЕ_VAPID_KEY' // Cloud Messaging → Web Push certificates
};
const PUSH_ENABLED = !FIREBASE_CONFIG.apiKey.includes('ВСТАВЬТЕ');

// ===== DEMO DATA (используется пока не подключён Google Sheets) =====
const DEMO_ORDERS = [
  {
...
let appBonuses = [];
let appPromos = [];
// Legacy alias so order modal still works
const orders = {};


// ================================================================
// API LAYER — запросы к Node.js Backend
// ================================================================
...
// Normalize raw Sheets row → app format
function normalizeOrder(raw) {
  // Handle both camelCase (demo) and snake_case (PostgreSQL)
  raw = {
...
// ================================================================
// DATA LOADER — загружает данные (API или demo)
// ================================================================
...
// Screens that have a nav button
...
function nav(screen) {
  ...
  // Activate nav button only if it exists
  const newNav = document.getElementById('nav-' + screen);
  if (newNav) newNav.classList.add('active');
  // If going to chat (from profile menu), highlight profile nav
  if (screen === 'chat') {
...
setGreeting();
// Bootstrap: load all data on startup — wrapped in DOMContentLoaded for Tilda compatibility
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', loadAppData);
} else {
  loadAppData(); // DOM already ready
}
...
function showReceiptModal(orderId) {
  // Find the order — use passed id or last opened
  ...
  if (o) {
    ...
    // Services list — show title as single service
...
// Простая функция числа прописью (только тысячи)
...
function generateQR(code) {
  ...
  // Simple deterministic QR pattern based on code
...
// ================================================================
// ЧАТ С МАСТЕРСКОЙ через Telegram Bot
// ================================================================

// Настройки — заполните после создания бота в @BotFather
const TG_CONFIG = {
...
// ---- Получение ответов мастера (polling) ----
async function pollTgReplies() {
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      // Мастер отвечает реплаем в Telegram — бот пересылает текст
...
// Перехватываем nav для инициализации чата
const _origNav = nav;
function nav(screen) {
  _origNav(screen);
  if (screen === 'chat') setTimeout(initChat, 100);
}
...
// Override saveSettings to also sync to backend
...
async function requestPushPermission() {
  ...
  try {
    ...
    if (token) {
      // Сохраняем токен локально и отправляем на сервер
...
// Обработка входящих push в фоне
function setupPushHandlers() {
  ...
  firebaseMessaging.onMessage((payload) => {
    ...
    if (title) {
      ...
      // Обновляем данные если изменился статус заказа
...
// ================================================================
// АВТОРИЗАЦИЯ — улучшенная версия с запросом push после входа
// ================================================================
// Переопределяем authStepVerify чтобы после входа запросить push
...
async function authStepVerify() {
  ...
  try {
    ...
    setToken(token);
    saveConsent();
    // После SMS — предлагаем PIN или входим
    afterSmsVerified();
    // После входа — предлагаем включить уведомления
...
function askPushPermission() {
  ...
  // Показываем красивый диалог вместо браузерного
  showPushDialog();
}
...
// Переопределяем toggleNotifications чтобы работал с FCM
async function toggleNotifications() {
  const u = loadUser();
  const allOn = u.notif_orders && u.notif_promos && u.notif_bday;
  if (!allOn) {
    // Включаем — запрашиваем разрешение браузера
    const token = await requestPushPermission();
    if (token) {
...
// Переопределяем saveSettings чтобы синхронизировал с сервером
...
// Стиль для анимации push-баннера
...
function handleLogoUpload(e) {
  ...
  reader.onload = function(ev) {
    ...
    // Persist in localStorage so logo survives reload
...
function formatPhone(input) {
  ...
  // Нормализуем: 8 → 7, добавляем 7 если нет кода страны
  if (digits.startsWith('8')) digits = '7' + digits.slice(1);
  if (!digits.startsWith('7') && digits.length > 0) digits = '7' + digits;
  digits = digits.slice(0, 11); // максимум 11 цифр

  // Строим маску +7 (___) ___-__-__
  let result = '';
  ...
  // Восстанавливаем курсор с поправкой на добавленные символы маски
...
// Инициализация маски при фокусе — сразу показываем +7
...
// Init: check auth on load — safe for Tilda
function _appInit() {
  const u = loadUser();
  applyUserToUI(u);
  if (!DEMO_MODE && !getToken()) {
    // If protection disabled and consent given — skip auth screen
    if (hasConsent && hasConsent() && !isPinRequired()) {
      // Auto-login: set a local session token
      setToken('local-session-' + Date.now());
      return; // loadAppData called after DOMContentLoaded
    }
...
// Consent already given — skip consent screen on next login
// Checkboxes are NEVER pre-checked — user must check them manually on first registration
function hasConsent() {
  try {
    const saved = localStorage.getItem('uvyantra_consent');
    if (!saved) return false;
    const c = JSON.parse(saved);
    return c.pd && c.terms; // both mandatory fields must be true
  } catch { return false; }
}

// showAuthScreen is defined in security section below


// ================================================================
// PIN-КОД
// ================================================================
let _pinTemp    = '';  // временный PIN при установке
let _pinConfirm = false; // ждём подтверждения
...
function handlePinComplete(mode, code) {
  if (mode === 'set-pin') {
    // Сохраняем временно, переходим к подтверждению
    _pinTemp = code;
    ...
    // Меняем PIN из профиля (без подтверждения старого)
    setPin(code);
    closePinChange();
    updatePinMenuLabel();
...
// Вход по PIN для повторных пользователей
function loginWithSmsInstead() {
  // Показываем телефон с пометкой что это восстановление PIN
  showPinStep('phone');
  const hint = document.getElementById('auth-otp-hint');
  // После успешного SMS — предложим установить новый PIN
  _pinRecovery = true;
}
let _pinRecovery = false;

// После успешного SMS-кода — проверяем нужно ли ставить PIN
function afterSmsVerified() {
  if (!hasPin() || _pinRecovery) {
    // Первая регистрация или восстановление PIN после SMS
    _pinRecovery = false;
    const title = document.querySelector('#auth-step-set-pin .auth-brand-like');
    showPinStep('set-pin');
    // Обновляем заголовок если это восстановление
...
// showAuthScreen unified — see security section
...
// ================================================================
// БЕЗОПАСНОСТЬ — вход без PIN / биометрия
// ================================================================
...
// ---- Переключатель "Открывать без авторизации" ----
...
// ---- Биометрия (WebAuthn / браузерный API) ----
...
function updateSecurityToggles() {
  ...
  // Скрываем биометрию если нет поддержки
...
// Обновлённый showAuthScreen с биометрией
function showAuthScreen() {
  ...
  if (isBiometricEnabled() && hasConsent()) {
    ...
    screen.classList.add('active');
    showPinStep('pin-login');
    // Show hint that SMS available for recovery
    ...
    // Защита отключена — входим сразу без экрана авторизации
    loadAppData();
    return;

  } else if (hasConsent()) {
    // Согласие есть, нет PIN — показываем телефон
...
// Добавляем кнопку биометрии на экран PIN-входа
